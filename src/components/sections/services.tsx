...
export default async function Services({ locale = "fa" }: ServicesProps) {
  const services = await apiClient.getServices(locale).catch(() => []);
    ...
    >
      <div className="max-w-[1440px] mx-auto px-6 lg:px-16">
        <div className="mb-16">
          ...
          <p className="text-lg text-neutral-500 leading-8">{t.description}</p>
        </div>
                ...
                <CardContent className="p-7">
                  <div
                    className={`w-12 h-12 rounded-xl flex items-center justify-center mb-5 transition-colors ${
                      s.highlight ? "bg-white/20" : "bg-primary-50 group-hover:bg-primary-100"
                    }`}
                  >
                    <Icon size={24} className={s.highlight ? "text-white" : "text-primary-500"} />
                  </div>

                  <CardTitle
                    className={`text-lg font-bold mb-3 ${s.highlight ? "text-white" : "text-neutral-900"}`}
                  >
                    {s.title}
                  </CardTitle>

                  <p className={`text-sm leading-7 mb-5 ${s.highlight ? "text-primary-100" : "text-neutral-500"}`}>
                    {s.summary}
                  </p>
                  ...
                  <Button
                    ...
                    className={`inline-flex items-center gap-1.5 text-sm font-semibold transition-all group/link ${
                      s.highlight ? "text-white/80 hover:text-white" : "text-primary-500 hover:text-primary-700"
                    }`}
                  >
